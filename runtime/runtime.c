...
void *Lclone (void *p) {
  ...
  if (UNBOXED(p)) {
...
#endif
      data *obj = (data*) alloc (sizeof(int) * (l+1));
...
#endif
      sexp *sobj = (sexp*) alloc (sizeof(int) * (l+2));
    ...
    default:
      ...
      res = NULL; // Чтобы избежать warning
    }
    pop_extra_root (&p);
...
# define COMPARE_AND_RETURN(x,y) do { if (x != y) return BOX((int)((x) - (y))); } while (0)
...
extern void* Belem (void *p, int i) {
  ...
  if (TAG(a->tag) == STRING_TAG) {
    return (void*) BOX((int)a->contents[i]);
  }
...
extern void* LmakeSexp (int bn, int btag) {
    sexp   *r;
    data   *d;

    ASSERT_UNBOXED("makeSexp:1", bn);
    ASSERT_UNBOXED("makeSexp:2", btag);
...
extern void* LMakeClosure (int bn, void *entry) {
    data    *r;
    int     n = UNBOX(bn);
...
extern int Btag (void *d, int t, int n) {
  ...
  if (UNBOXED(d)) return BOX(0);
    ...
    r = TO_DATA(d);
#ifndef DEBUG_PRINT
    return BOX(TAG(r->tag) == SEXP_TAG && TO_SEXP(d)->tag == UNBOX(t) && LEN(r->tag) == (unsigned)UNBOX(n));
#else
    return BOX(TAG(r->tag) == SEXP_TAG &&
               GET_SEXP_TAG(TO_SEXP(d)->tag) == UNBOX(t) && LEN(r->tag) == (unsigned)UNBOX(n));
#endif
  }
}
...
extern int Barray_patt (void *d, int n) {
  ...
  if (UNBOXED(d)) return BOX(0);
    ...
    r = TO_DATA(d);
    return BOX(TAG(r->tag) == ARRAY_TAG && LEN(r->tag) == (unsigned)UNBOX(n));
  }
}
...
extern FILE* Lfopen (char *f, char *m) {
  ...
  return NULL; // Чтобы избежать warning: control reaches end of non-void function
}
